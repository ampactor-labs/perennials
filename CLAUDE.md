...
to load into context. To work on the data path, read `server/`, and `scripts/data/` for
the original build-time pipeline it replaced.

## Layout

- `src/` the app: `components/`, `pages/`, `state/`, `lib/`, `data/`, `styles/`
- `server/` the data API (Node + Postgres): pull, transform, ingest, serve
- `scripts/data/` the original build-time pull and transform, kept for reference
