...
// One input that speaks the whole grammar: type "wet" and get the Water: Wet
// constraint; type "mulberry" and jump to the plant; Enter keeps free text.
export function Omnibox() {
  ...
  const suggestions = useMemo<Suggestion[]>(
    () => (s.data && draft.trim() ? suggest(s.data, lexicon, draft) : []),
    [s.data, lexicon, draft],
  );
  ...
  function onKeyDown(e: React.KeyboardEvent) {
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      const sug = suggestions[cursor];
      if (sug && draft.trim()) pick(sug);
      else {
        s.setText(draft);
        setOpen(false);
      }
      ...
      setOpen(false);
    } else if (e.key === "Backspace" && draft === "" && s.constraints.atoms.length > 0) {
      // Backspace on empty input pops the newest constraint — token-input muscle memory.
    ...
    <div className="omnibox" ref={boxRef}>
      ...
      {showDrop && (
        <div className="omni-drop" id="omnibox-listbox" role="listbox">
          ...
          <div className="omni-hint">Enter searches names · picking a suggestion adds it as a filter</div>
        </div>
      )}
    </div>
  );
